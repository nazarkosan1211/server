...
def ensure_columns():
    with engine.connect() as conn:
        ...
        migrations = {
            ...
            "last_task_time": "ALTER TABLE users ADD COLUMN IF NOT EXISTS last_task_time INTEGER DEFAULT 0",
            "task_token": "ALTER TABLE users ADD COLUMN IF NOT EXISTS task_token VARCHAR",
            "task_token_time": "ALTER TABLE users ADD COLUMN IF NOT EXISTS task_token_time INTEGER DEFAULT 0",
            ...
            "daily_streak": "ALTER TABLE users ADD COLUMN IF NOT EXISTS daily_streak INTEGER DEFAULT 0",
            "last_checkin_day": "ALTER TABLE users ADD COLUMN IF NOT EXISTS last_checkin_day VARCHAR DEFAULT ''",
            "joined_channel_claimed": "ALTER TABLE users ADD COLUMN IF NOT EXISTS joined_channel_claimed INTEGER DEFAULT 0",
            "is_banned": "ALTER TABLE users ADD COLUMN IF NOT EXISTS is_banned INTEGER DEFAULT 0",
            "admin_note": "ALTER TABLE users ADD COLUMN IF NOT EXISTS admin_note VARCHAR",
            "ip_address": "ALTER TABLE users ADD COLUMN IF NOT EXISTS ip_address VARCHAR",
            "device_id": "ALTER TABLE users ADD COLUMN IF NOT EXISTS device_id VARCHAR",
            "suspicious_score": "ALTER TABLE users ADD COLUMN IF NOT EXISTS suspicious_score INTEGER DEFAULT 0",
            "suspicious_reason": "ALTER TABLE users ADD COLUMN IF NOT EXISTS suspicious_reason VARCHAR",
            "last_seen_at": "ALTER TABLE users ADD COLUMN IF NOT EXISTS last_seen_at TIMESTAMP",
        }
...
def is_user_in_channel(user_id):
    try:
        ...
        response = requests.get(url, params={"chat_id": CHANNEL_USERNAME, "user_id": user_id}, timeout=15)
        data = response.json()
        if not data.get("ok"):
            return False, data
        status = data.get("result", {}).get("status", "")
        return status in ["member", "administrator", "creator"], data
...
def start_user():
    ...
    if not user:
        user = User(user_id=user_id, username=username, ref_by=ref if ref and ref != user_id else None, last_reset_day=reset_day_wib())
        session.add(user)
        session.commit()
    ...
    result = {"status": "success", "user_id": user_id, "username": username, **user_response(user)}
    session.close()
...
def start_task():
    ...
    if user.tasks_done >= DAILY_TASK_LIMIT:
        session.close()
        return jsonify({"status": "blocked", "reason": "daily_limit", "remaining_tasks": 0})
    ...
    return jsonify({"status": "success", "task_token": token, "expires_in": TASK_TOKEN_EXPIRE})
...
def add_coin():
    ...
    if not task_token:
        session.close()
        return jsonify({"status": "blocked", "reason": "missing_task_token", "message": "Invalid claim"}), 403

    if not user.task_token or task_token != user.task_token:
        session.close()
        return jsonify({"status": "blocked", "reason": "invalid_task_token", "message": "Invalid claim"}), 403
    ...
    if now - int(user.task_token_time or 0) > TASK_TOKEN_EXPIRE:
        ...
        session.commit()
        session.close()
        return jsonify({"status": "blocked", "reason": "expired_task_token", "message": "Task expired"}), 403

    if user.tasks_done >= DAILY_TASK_LIMIT:
        result = {"status": "blocked", "reason": "daily_limit", "wait": 0, **user_response(user)}
...
def claim_super_task():
    ...
    if not super_task_token:
        session.close()
        return jsonify({"status": "blocked", "reason": "missing_super_task_token", "message": "Invalid claim"}), 403

    if not user.super_task_token or super_task_token != user.super_task_token:
        session.close()
        return jsonify({"status": "blocked", "reason": "invalid_super_task_token", "message": "Invalid claim"}), 403
    ...
    if now - int(user.super_task_token_time or 0) > SUPER_TASK_TOKEN_EXPIRE:
        ...
        session.commit()
        session.close()
        return jsonify({"status": "blocked", "reason": "expired_super_task_token", "message": "Super Reward expired"}), 403
    ...
    result = {"status": "success", "reward": SUPER_TASK_REWARD, **user_response(user)}
    session.close()
    return jsonify(result)
...
def claim_checkin():
    ...
    if user.last_checkin_day == today_key:
        result = {"status": "blocked", "reason": "already_claimed", "message": "Daily check-in already claimed", **user_response(user)}
        session.commit()
        session.close()
        return jsonify(result)

    streak = int(user.daily_streak or 0) + 1 if user.last_checkin_day == yesterday_key else 1
    if streak > 7:
        streak = 1
    ...
    result = {"status": "success", "reward": reward, **user_response(user)}
    session.close()
    return jsonify(result)
...
def verify_channel():
    ...
    if int(user.joined_channel_claimed or 0) == 1:
        result = {"status": "blocked", "reason": "already_claimed", "message": "Channel reward already claimed", **user_response(user)}
        session.commit()
        session.close()
        return jsonify(result)
    ...
    if not is_member:
        result = {"status": "blocked", "reason": "not_joined", "message": "Please join the channel first", "telegram_response": tg_response, **user_response(user)}
        session.commit()
        session.close()
        return jsonify(result)
    ...
    result = {"status": "success", "reward": CHANNEL_REWARD, "message": "Channel task completed", **user_response(user)}
    session.close()
    return jsonify(result)
...
def leaderboard():
    session = SessionLocal()
    users = session.query(User).order_by(User.coins.desc()).limit(20).all()
    result = []
    for index, user in enumerate(users, start=1):
        name = user.username if user.username else f"User {user.user_id[-4:]}"
        result.append({"rank": index, "user_id": user.user_id, "username": name, "coins": user.coins})
    session.close()
...
def request_withdraw():
    ...
    if amount < MIN_WITHDRAW_POINTS:
        return jsonify({"status": "blocked", "reason": "minimum", "message": f"Minimum withdraw is {MIN_WITHDRAW_POINTS} points"}), 400
    ...
    if int(user.coins or 0) < amount:
        session.close()
        return jsonify({"status": "blocked", "reason": "insufficient_points", "message": "Not enough points"}), 400

    pending = session.query(WithdrawRequest).filter(WithdrawRequest.user_id == user_id).filter(WithdrawRequest.status == "pending").first()
    if pending:
        session.close()
        return jsonify({"status": "blocked", "reason": "pending_exists", "message": "You already have a pending withdraw request"}), 400

    user.coins -= amount
    wd = WithdrawRequest(user_id=user.user_id, username=user.username, method=method, account=account, amount=amount, status="pending")
    session.add(wd)
    session.commit()

    result = {"status": "success", "message": "Withdraw request submitted", "withdraw_id": wd.id, "coins": user.coins, **user_response(user)}
    session.close()
    return jsonify(result)
...
def my_withdraws():
    ...
    session = SessionLocal()
    rows = session.query(WithdrawRequest).filter(WithdrawRequest.user_id == user_id).order_by(WithdrawRequest.created_at.desc()).limit(20).all()
    result = []
    for w in rows:
        result.append({"id": w.id, "method": w.method, "account": w.account, "amount": int(w.amount or 0), "status": w.status, "admin_note": w.admin_note or "", "created_at": str(w.created_at) if w.created_at else "", "updated_at": str(w.updated_at) if w.updated_at else ""})
    session.close()
    return jsonify({"status": "success", "withdraws": result})
...
def admin_users():
    ...
    if q:
        like = f"%{q}%"
        query = query.filter((User.user_id.ilike(like)) | (User.username.ilike(like)) | (User.ref_by.ilike(like)))
...
def admin_suspicious():
    if not admin_ok(request):
        return jsonify({"status": "error", "message": "Invalid admin key"}), 403
    session = SessionLocal()
    users = session.query(User).filter(User.suspicious_score >= 30).order_by(User.suspicious_score.desc(), User.coins.desc()).limit(200).all()
    result = [admin_user_json(user) for user in users]
    session.close()
    return jsonify({"status": "success", "users": result})
...
def admin_withdraws():
    ...
    result = []
    for w in rows:
        result.append({"id": w.id, "user_id": w.user_id, "username": w.username or "", "method": w.method or "", "account": w.account or "", "amount": int(w.amount or 0), "status": w.status, "admin_note": w.admin_note or "", "created_at": str(w.created_at) if w.created_at else "", "updated_at": str(w.updated_at) if w.updated_at else ""})
    session.close()
    return jsonify({"status": "success", "withdraws": result})
...
def admin_update_withdraw():
    ...
    session.commit()
    result = {"id": wd.id, "status": wd.status, "user_id": wd.user_id, "amount": int(wd.amount or 0)}
    session.close()
    return jsonify({"status": "success", "withdraw": result})
...
def admin_referrals():
    if not admin_ok(request):
        return jsonify({"status": "error", "message": "Invalid admin key"}), 403
    session = SessionLocal()
    users = session.query(User).filter(User.ref_by != None).order_by(User.created_at.desc()).limit(200).all()
    result = []
    for user in users:
        result.append({"new_user_id": user.user_id, "new_username": user.username or "", "ref_by": user.ref_by or "", "created_at": str(user.created_at) if user.created_at else ""})
    session.close()
    return jsonify({"status": "success", "referrals": result})
...
def debug_reset_time():
    now_wib = datetime.utcnow() + timedelta(hours=7)
    return jsonify({"status": "success", "now_wib": now_wib.strftime("%Y-%m-%d %H:%M:%S"), "reset_hour_wib": RESET_HOUR_WIB, "current_reset_day": reset_day_wib()})

@app.route("/debug_channel", methods=["GET"])
def debug_channel():
    return jsonify({"status": "success", "channel_username": CHANNEL_USERNAME, "channel_reward": CHANNEL_REWARD})
